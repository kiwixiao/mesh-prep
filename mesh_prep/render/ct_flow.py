...
"""
4D-Flow-Style CT + CFD visualization: CT volume rendering with streamlines.

Renders RANS CFD streamlines inside a CT volume rendering of the aorta,
mimicking 4D Flow MRI visualization. The CT lumen is transparent while
vessel walls and bone are semi-opaque, providing anatomical context for
the flow field.

Uses the same 4-panel anatomical layout as other render scripts (Z-up):
...
Usage:
    ...
    # Render all frames:
    $PVBATCH ct_flow.py --case /path/to/AO_Native_001_RANS --ct-file /path/to/CT.vti

    # Fixed colorbar range + custom CT opacity:
    $PVBATCH ct_flow.py --case /path/to/AO_Native_001_RANS --ct-file /path/to/CT.vti --range 0 3 --ct-opacity 0.5

    # Render frame range (for parallel workers):
    $PVBATCH ct_flow.py --case /path/to/AO_Native_001_RANS --ct-file /path/to/CT.vti --start 0 --end 40

    # Query total frame count:
    $PVBATCH ct_flow.py --case /path/to/AO_Native_001_RANS --total-frames

    # Debug: skip CT volume rendering:
    $PVBATCH ct_flow.py --case /path/to/AO_Native_001_RANS --no-ct
"""
...
# CLI
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="4D-Flow-Style CT + CFD visualization")
...
OUTPUT_DIR = os.path.join(os.path.dirname(CASE_DIR),
                          f"{os.path.basename(CASE_DIR)}_renders", "ct_flow")
...
# Create 2x2 layout
# ---------------------------------------------------------------------------
layout = CreateLayout("4-Panel CT + Flow")
...
# ---------------------------------------------------------------------------
print(f"\nLoading {FOAM_FILE} (internalMesh) ...")
...
if not args.no_ct:
    if os.path.exists(args.ct_file):
        ...
        print("  Run: conda run -n mesh-prep python nii_to_vti.py")
...
print(f"\nColorbar range: [{vel_min:.4g}, {vel_max:.4g}]")
...
# ---------------------------------------------------------------------------
# CT Transfer Function — X-ray style: grayscale, mostly transparent, bones only
# ---------------------------------------------------------------------------
ct_lut = None
ct_pwf = None
...
for name, view in views.items():
    ...
    # CT volume rendering (behind streamlines — rendered first)
    if ct_source is not None:
        ...
        # Set volume property for trilinear interpolation
        vp = ct_display.GetProperty("VolumeProperty")
        if vp is not None:
            vp.SetInterpolationType(1)  # 1 = linear
...
# Force LUT range AFTER all ColorBy calls (prevents auto-rescale override)
lut.RescaleTransferFunction(vel_min, vel_max)
lut.AutomaticRescaleRangeMode = "Never"
...
print(f"\nRendering {n_frames} CT+flow frames [{frame_start}..{frame_end - 1}] to {OUTPUT_DIR}/ ...")
