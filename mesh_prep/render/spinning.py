...
"""
Spinning 3D overview — single-panel orbiting camera movie.

The camera orbits around the aorta (Z-up) while time advances, producing a
video that shows both temporal flow evolution and full spatial context.
...
Supports two rendering modes:
  - volume:      Gaussian-smoothed volume rendering of velocity
  - streamlines: Tube streamlines seeded from inlet

Usage:
    PVBATCH=/Applications/ParaView-6.0.1.app/Contents/bin/pvbatch

    # Spinning streamlines (250 frames, 25fps, 10s, 2 orbits):
    $PVBATCH spinning.py --case /path/to/AO_Native_001_RANS --mode streamlines --range 0 3

    # Custom fps/duration:
    $PVBATCH spinning.py --case /path/to/AO_Native_001_RANS --mode volume --fps 30 --duration 15

    # Single-frame test (--start/--end are output frame indices):
    $PVBATCH spinning.py --case /path/to/AO_Native_001_RANS --mode streamlines --start 0 --end 1

    # Query total frame count (for parallel splitting):
    $PVBATCH spinning.py --total-frames
"""
...
# CLI
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Spinning 3D overview movie")
...
parser.add_argument("--mode", type=str, default="volume",
                    ...
                    help="Rendering mode (default: volume)")
parser.add_argument("--range", type=float, nargs=2, default=None,
                    metavar=("MIN", "MAX"),
                    help="Manual colorbar range")
...
parser.add_argument("--resample", type=int, default=256,
                    help="ResampleToImage grid size for volume mode (default: 256)")
parser.add_argument("--smooth", type=float, default=4.0,
                    help="Gaussian smooth sigma for volume mode (default: 4.0, 0=off)")
...
parser.add_argument("--tube-radius", type=float, default=0.0001,
                    help="Tube radius for streamlines mode (default: 0.0001)")
...
OUTPUT_DIR = os.path.join(os.path.dirname(CASE_DIR),
                          f"{os.path.basename(CASE_DIR)}_renders", f"spinning_{args.mode}")
RESOLUTION = [1920, 1080]  # single-panel 1080p
...
# Resolve output frame range (--start/--end refer to output frame indices)
...
if args.mode == "volume":
    ...
    vel_max = args.range[1] if args.range else 5.0

    N = args.resample
    print(f"Creating ResampleToImage filter ({N}^3 grid, smooth sigma={args.smooth}) ...")
    resample = ResampleToImage(Input=foam)
...
"""
    ...
    SetActiveView(view)
    display = Show(volume_source, view)
    display.Representation = "Volume"
    ColorBy(display, color_spec)
    display.LookupTable = lut
    display.VolumeRenderingMode = "GPU Based"
    vp = display.GetProperty("VolumeProperty")
    if vp is not None:
        vp.SetInterpolationType(1)
...
scalar_bar.Title = bar_title
scalar_bar.ComponentTitle = bar_unit
...
# Mode label
mode_label = Text()
mode_label.Text = f"Spinning 3D — {args.mode.title()}"
...
print(f"\nRendering {n_frames} spinning frames, {args.orbits} orbit(s) ...")
...
prev_ts_idx = -1  # track timestep changes to avoid redundant pipeline updates
...
if args.start is None and args.end is None:
    ...
    print(f"      spinning_{args.mode}.mp4")
